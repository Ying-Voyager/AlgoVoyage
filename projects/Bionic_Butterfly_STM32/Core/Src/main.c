...
/**
  ...
  * - 通道2 (右摇杆Y): 控制扑翼幅度 (上推=幅度增大, Mode2:90~150度)
  * - 通道3 (左摇杆Y): 控制扑翼频率 (上推=频率增大, Mode2:3~10Hz)
  * - 通道4 (左摇杆X): 控制前后差速俯仰 (右推=前翼加速=抬头, Mode2)
  * - 通道5 (拨杆F): 启动/停止开关 (上=启动, 下=停止)
  * - 通道6 (拨杆B): 模式选择 (上=Mode2飞行, 中=Mode1立翅, 下=Mode0收翅)
  * 
  * Mode1特殊用途:
  * - 通道2 (右摇杆Y): 微调立翅角度 (±80, 用于调试motor_Front/Rear_midpoint)
  *
  ******************************************************************************
  */
...
// 用于静态立翅模式，翅膀竖直向上
int16_t motor_LF_standing = 1690;  // 左前翼立翅位置（需调试确定）
int16_t motor_RF_standing = 1690;  // 右前翼立翅位置（需调试确定）
int16_t motor_LR_standing = 3150;  // 左后翼立翅位置（需调试确定）
int16_t motor_RR_standing = 3150;  // 右后翼立翅位置（需调试确定）

// ----- Mode2: 扑翼基准点（水平，约0度）-----
// 用于飞行模式的扑翼中心，翅膀水平位置
int16_t motor_LF_flying = 2300;    // 左前翼扑翼中心（需调试确定）
int16_t motor_RF_flying = 2300;    // 右前翼扑翼中心（需调试确定）
int16_t motor_LR_flying = 2330;    // 左后翼扑翼中心（需调试确定）
int16_t motor_RR_flying = 2330;    // 右后翼扑翼中心（需调试确定）

// ----- Mode0: 收翅基准点（收起，约45度）-----
// 用于收翅模式，翅膀收起贴近身体
int16_t motor_LF_folded = 2000;    // 左前翼收翅位置（需调试确定）
int16_t motor_RF_folded = 2000;    // 右前翼收翅位置（需调试确定）
int16_t motor_LR_folded = 2600;    // 左后翼收翅位置（需调试确定）
int16_t motor_RR_folded = 2600;    // 右后翼收翅位置（需调试确定）

// 调试提示：
// 1. 手动将左前翼调到竖直位置，读取编码器1的值 → 填入motor_LF_standing
// 2. 手动将右前翼调到竖直位置，读取编码器3的值 → 填入motor_RF_standing
// 3. 手动将左后翼调到竖直位置，读取编码器2的值 → 填入motor_LR_standing
// 4. 手动将右后翼调到竖直位置，读取编码器4的值 → 填入motor_RR_standing
// 5. 重复上述步骤，分别测量水平位置（flying）和收翅位置（folded）
// 
//电机布局为 M1(左前) M3(右前)
//           M2(左后) M4(右后)
// ========================================
...
// 2 = 后翼落后约40度
// 4 = 后翼落后约80度（推荐）
int8_t phase_offset = 2;  // 调试时可以修改这个值

// Mode2扫翅开关状态（使用拨杆8控制）
static uint8_t mode2_flapping = 0;  // 0=悉停在水平位置, 1=正在扫翅
  ...
  while (1)
  {
    ...
    if (elrs_data.Switch == 1)
    {
        ...
        motor_4_pid.Kp = 30;
			...
			// 遥控器通道映射
			// elrs_data.Throttle  -> 整体频率 (通道3, 左摇杆Y)  范围: 3~10Hz
			// elrs_data.Yaw       -> 前后频率差 (通道4, 左摇杆X)  范围: -100~100
			// elrs_data.midpoint  -> 整体幅度 (通道2, 右摇杆Y)  范围: -80~80
			// elrs_data.Roll      -> 左右幅度差 (通道1, 右摇杆X)  范围: -400~400
			...
			// ============ 频率控制算法（前后差速俯仰）============
			// Step1: 左摇杆Y控制整体频率 (3~10Hz)
			int16_t base_freq = (int16_t)elrs_data.Throttle;
			if (base_freq < 3) base_freq = 3;
			if (base_freq > 10) base_freq = 10;
			...
			// Step4: 硬限制在1~10Hz范围内
			if (freq_front < 1) freq_front = 1;
			if (freq_front > 10) freq_front = 10;
			if (freq_rear < 1) freq_rear = 1;
			if (freq_rear > 10) freq_rear = 10;
			...
			if ((int32_t)(tick - sm_next_tick_front) >= 0)
			{
				...
				// 前翼目标角度（4个电机各自使用自己的基准点）
				Wings_Data.Wings_motor[0].Target_Angle = (int16_t)(base_LF - q15_mul(amp_L_encoder, c_front));  // 左前
				Wings_Data.Wings_motor[1].Target_Angle = (int16_t)(base_RF - q15_mul(amp_R_encoder, c_front));  // 右前
			...
			if ((int32_t)(tick - sm_next_tick_rear) >= 0)
			{
				...
				// 后翼目标角度（4个电机各自使用自己的基准点）
				Wings_Data.Wings_motor[2].Target_Angle = (int16_t)(base_LR - q15_mul(amp_L_encoder, c_rear));  // 左后
				Wings_Data.Wings_motor[3].Target_Angle = (int16_t)(base_RR - q15_mul(amp_R_encoder, c_rear));  // 右后
        ...
        else
		{
			   ...
			   abs16_fast(target_LR - Wings_Data.Wings_motor[2].Corrective_Angle) > 200)
			{
				...
				motor_2_pid.Kp = motor_1_pid.Kp;
				
				// 后翼的Kp根据后前距离比例调整
				motor_3_pid.Kp = 12 * (float)abs16_fast(target_LR - Wings_Data.Wings_motor[2].Corrective_Angle) / 
				                      (float)abs16_fast(target_LF - Wings_Data.Wings_motor[0].Corrective_Angle);	
				motor_4_pid.Kp = motor_3_pid.Kp;
			}
