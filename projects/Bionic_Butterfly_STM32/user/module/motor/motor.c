...
void Motor_PID_Control(void)
{
	...
	// M3 右前翼（物理输出到motor_2）
	Wings_Data.Wings_motor[1].Target_Speed = motor_2_set_pwm = PID_calc(&motor_2_pid, 
		Wings_Data.Wings_motor[1].Corrective_Angle, 
		Wings_Data.Wings_motor[1].Target_Angle);
	
	// M2 左后翼（物理输出到motor_3）
	Wings_Data.Wings_motor[2].Target_Speed = motor_3_set_pwm = -PID_calc(&motor_3_pid, 
	...
	// 调用时交换motor_2和motor_3顺序以匹配硬件布线
	Set_Pwm(motor_1_set_pwm, motor_3_set_pwm, motor_2_set_pwm, motor_4_set_pwm);
}
...
// 硬件PWM映射：m1→左前, m2→左后(PWM_M4), m3→右前(PWM_M3), m4→右后(PWM_M2)
